...
if not GROQ_API_KEY:
    st.error("Missing key, Please run the $env command")
    st.stop()
...
# function to convert text to speech (initialization)
def get_tts_engine():
    try:
        engine = pyttsx3.init()
        return engine
...
def speak(text, voice_gender = "girl"):
    try:
        ...
        st.error(f"TTS related Error: {e}")
...
def listen_to_speech():
    try:
        ...
        text = recognizer.recognize_google(audio)  # using google API function to convert speech to text
        return text.lower()
...
# Function to get AI response from Groq LLm
def get_ai_response(messages):
    try:
        ...
        return result.strip() if result else "Sorry, I was not able to generate content"
    except Exception as e:
        return f"Error while getting AI response : {e}"
...
def main():
    ...
    # this is setting rules & instructions for the LLM
    if "chat_history" not in st.session_state:
        st.session_state.chat_history = [
            {"role" : "system",     # this is setting rules & instructions for the LLM
             "content" : "You are a helpful voice assistant. Reply just one line. Speak like Bhramanandam"
             }
        ]

    # Initialize the message you want to pass
    ...
    with st.sidebar:
        ...
        # If disabled then no voice just text reply from voice assistant
    ...
    # Putting welcome in the start
    if not st.session_state.messages:
        st.info("Welcome : Use voice input button or Type message to start chatting")

    st.markdown("---")
    st.markdown(
        """
        <div style = 'text-align: center; color: #666;'>
        <p> Voice Assistant Project * Copy right to Puneet Kansal </p>
        </div>
        """,
